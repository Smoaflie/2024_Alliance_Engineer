...
#define UI_Graph_Refresh_freq 2
...
uint8_t UI_Graph_Setting(UI_GRAPH_INSTANCE *instance, RefreshMode_ mode){
    ...
    switch(instance->type){
        ...
        case GraphType_Line:
            uint32_t Line_end_x = instance->pos_x - instance->param.Line.length*sin(instance->param.Line.rotate_angle*DEGREE_2_RAD);
            uint32_t Line_end_y = instance->pos_y + instance->param.Line.length*cos(instance->param.Line.rotate_angle*DEGREE_2_RAD);
        ...
        case GraphType_Arc:
            uint32_t start_angle = instance->param.Arc.start_angle;
            uint32_t end_angle = instance->param.Arc.start_angle + instance->param.Arc.angle_len;
        ...
        case GraphType_Float:
            uint32_t value = (int32_t)(instance->param.Float.value * 1000) / (10*instance->param.Float.digit) * (10*instance->param.Float.digit);
...
UI_GRAPH_INSTANCE* UI_Graph_Init(
    ...
    switch(config.type){
        case GraphType_Rect:
            config.param.Rect.width = va_arg(ap, uint32_t);
            config.param.Rect.height = va_arg(ap, uint32_t);
            break;
        case GraphType_Line:
            config.param.Line.length = va_arg(ap, uint32_t);
            config.param.Line.rotate_angle = va_arg(ap, double);
            break;
        case GraphType_Round:
            config.param.Round.radius = va_arg(ap, uint32_t);
            break;
        case GraphType_Elipse:
            config.param.Ellipse.radius_x = va_arg(ap, uint32_t);
            config.param.Ellipse.radius_y = va_arg(ap, uint32_t);
            break;
        case GraphType_Arc:
            config.param.Arc.start_angle = va_arg(ap, uint32_t);
            config.param.Arc.angle_len = va_arg(ap, uint32_t);
            config.param.Arc.radius_x = va_arg(ap, uint32_t);
            config.param.Arc.radius_y = va_arg(ap, uint32_t);
            break;
        case GraphType_Number:
            config.param.Number.size = va_arg(ap, uint32_t);
            config.param.Number.value = va_arg(ap, int32_t);
            break;
        case GraphType_Float:
            config.param.Float.size = va_arg(ap, uint32_t);
            config.param.Float.digit = va_arg(ap, uint32_t);
    ...
    for(int i = UI_String_PRIORITY_LEVEL-1; i >= 0; i--){
        for(int j = 0; j < graph_idx_priority[i]; j++){
            ...
            if(string_instance_group[i][j]->send_flag && --string_instance_group[i][j]->delay<=0){
                ...
                string_instance_group[i][j]->delay = UI_Graph_Refresh_freq;
